...
class Board {
  ...
  copy() {
    // Create a new board without allocating a new array in the constructor
    const newBoard = Object.create(Board.prototype)
    newBoard.width = this.width
    newBoard.height = this.height
    newBoard.array = new Int8Array(this.array)
    return newBoard
  }
  at(x, y) {
...
function move(board, x, y, x1, y1) {
  ...
  // Current player's turn is being used
  b.turn = -board.turn
...
function movesVals(board, depth) {
  const v = moves(board)
  for (let b of v) b.val = minimax(b, depth)
...
function verify(board) {
  ...
  if (width < minWidth) throw new Error(width)
  if (height < minHeight) throw new Error(height)
  if (array.length !== width * height) throw new Error(array.length)
  ...
  for (const piece of array) pieceChar(piece)
  ...
  for (let x = 0; x < width; x++)
    if (Math.abs(board.at(x, 0)) === PAWN || Math.abs(board.at(x, height - 1)) === PAWN) throw new Error(x)
}
