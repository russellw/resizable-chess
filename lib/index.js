...
function validMoves(board) {
  ...
  function pawn() {
    ...
    if (turn >= 0) {
      ...
      let x2 = x1 - 1
      if (x2 >= 0) {
        if (get(board, x2, y1) < 0) add1(x2, y1)
      }
...
function staticVal(board) {
  ...
  const height = board.height
  let r = 0.0
  for (let y = 0; y < height; y++) for (let x = 0; x < width; x++) r += pieceVal(board, get(board, x, y))
  return r
}

function emptyBoard(width, height) {
  if (!(minWidth <= width && width <= 0xff)) throw new Error(width)
  if (!(minHeight <= height && height <= 0xff)) throw new Error(height)
...
function verify(board) {
  ...
  for (let x = 0; x < width; x++)
    if (Math.abs(get(board, x, 0)) == PAWN || Math.abs(get(board, x, height - 1)) == PAWN) throw new Error(x)
}
