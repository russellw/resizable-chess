...
function move(board, m) {
  ...
  for (let Y = 0; Y < height; Y++) {
    //new board has reverse perspective, for next minimax step
    //so it is flipped vertically
    const Y1 = height - 1 - Y
    for (let X = 0; X < width; X++) {
      ...
      if (Y == y1 && X == x1) {
        piece = get(board, x, y)
        if (piece == PAWN && y1 == height - 1) piece = QUEEN
      } else piece = get(board, X, Y)

      //new board has reverse perspective, for next minimax step
      //so piece changes color
      put(b, X, Y1, piece)
    }
  }
  return b
...
function validMoves(board) {
  ...
  function add(x1, y1) {
    ...
    if (get(board, x1, y1) > 0) return false
  ...
  function pawn() {
    ...
    const limit = y1 == 1 ? 2 : Math.floor(height / 2)
    ...
    //capture
    y1 = y + 1
    if (x1 && get(board, x1 - 1, y1) < 0) add(x1 - 1, y1)
    if (x1 + 1 < width && get(board, x1 + 1, y1) < 0) add(x1 + 1, y1)
  }
...
function initialBoard(width, height) {
  ...
  const knights = Math.ceil((pieces - rooks) / 2)
