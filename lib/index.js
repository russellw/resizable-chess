...
function minimax(board, depth, maxDepth) {
  ...
  if (board.turn >= 0) {
    let r = -Infinity
    for (let i = 0; i < v.length; i++) r = Math.max(r, minimax(v[i], depth, maxDepth))
    return r
  } else {
    let r = Infinity
    for (let i = 0; i < v.length; i++) r = Math.min(r, minimax(v[i], depth, maxDepth))
    return r
  }
}
