...
function printBoard(board, dent = 0) {
  // Separator
  indent(dent)
...
function printTree(board, dent = 0) {
  printBoard(board, dent)
  dent += 2
  if (board.moves) for (let b of board.moves) printTree(b, dent)
}
...
function valid(board) {
  ...
  // Can't have both players in check
  if (check(board, 1) && check(board, -1)) return false
