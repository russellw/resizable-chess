...
function minimax(board, depth, maxDepth, alpha, beta) {
  nodes++
  if (depth === maxDepth) return staticVal(board)
  const v = moves(board)
  let val = 0.0
  if (board.turn >= 0) {
    val = -Infinity
    for (let i = 0; i < v.length; i++) {
      val = Math.max(val, minimax(v[i], depth + 1, maxDepth, alpha, beta))
      if (beta <= val) break
    ...
    val = Infinity
    for (let i = 0; i < v.length; i++) {
      val = Math.min(val, minimax(v[i], depth + 1, maxDepth, alpha, beta))
      if (val <= alpha) break
...
function movesVals(board, maxDepth) {
  ...
  for (let i = 0; i < v.length; i++) {
    const b = v[i]
    b.val = minimax(b, 1, maxDepth, -Infinity, Infinity)
  }
  return v
}
