...
class Board {
  ...
  copy() {
    const board = new Board(this.width, this.height, -this.turn)
  ...
  str() {
    ...
    const v = []
    for (let i = 0; i < array.length; i++) v.push(pieceChar(array[i]))
    if (this.turn < 0) v.push("-")
    return v.join("")
  }
}
...
function decodeFEN(width, height, fen) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    for (let j = 0; j < row.length; j++) {
      ...
      const piece = charPiece.get(c)
      if (piece === undefined) throw new Error(c)
      board.put(x, y, piece)
      x++
    }
    y--
...
function emptyBoard(width, height) {
  const board = new Board(width, height, 1)
